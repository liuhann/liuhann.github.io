...
function ready() {
	$("#iphone").deviceshell({
		width: $(window).height()/2-10
	});

	$("#iphone").show();

	if (isMobile) {
		$("#iphone").css("font-size", Math.floor($(window).width()/20));
	} else {
		$("#iphone").css("font-size", Math.floor($(window).height()/50));
	}
...
function initData() {
	...
	if (!tagdb().first()) {
		tagdb.insert({"bg": "#4682B4","font":"#fff","title":"Pastime"});
		tagdb.insert({"bg": "#48D1CC","font":"#fff","title":"Sports"});
		tagdb.insert({"bg": "#BA55D3","font":"#fff","title":"Way"});
		tagdb.insert({"bg": "#FF69B4","font":"#fff","title":"Talk"});
		tagdb.insert({"bg": "#8B0000","font":"#fff","title":"Rest"});
		tagdb.insert({"bg": "#9ACD32","font":"#fff","title":"Study"});
		tagdb.insert({"bg": "#FF8C00","font":"#fff","title":"Work"});
	}
}
...
function initEvent() {
	...
	$("#add").attach(function() {
		if ($(".add-menu.slidedown").length>0) {
			$(".add-menu").removeClass("slidedown");
		} else {
			clearPop();
			$(".add-menu").addClass("slidedown");
		}
	});
...
function pageTiming(d) {
	...
	$(".content").css("-webkit-transform", "translateX(0)");
...
function openPage(container) {
	$(container).css("opacity", 0);
	$(container).show();
	$(container).css("opacity", 1);
	$(container).find('ul.list').css("-webkit-transform", "scale(1)");
}
...
function confirmCurrent(cb) {
	var c = getCurrentRunning();
	if (c) {
		if (confirm('There is a timer running, stop it?')) {
			stop();
			cb();
		}
	} else {
...
function pageTodo() {
	...
	function addTodoItem(record) {
		...
		cloned.find(".todocontent").html(record.type + ":" + record.desc);
		cloned.find(".todocontent").css("color", getTypeColor(record.type));
...
function clearPop() {
	$(".header .add-menu").removeClass("slidedown");
	//$(".main .todo").hide();
}
...
function pageChart() {
	...
	$(".content").css("-webkit-transform", "translateX(-50%)");
...
function drawDateDonut(time, dateDura) {
    ...
    $(".chart .title .datecontent").html(chartTitle);
    ...
    if (total == 0 ) {
    	data.push({value:ONE_DAY_MILL, color: "#ccc", label: "None"});
    }
...
function pageMe() {
	...
	$(".content").css("-webkit-transform", "translateX(-75%)");
	$(".content").css("transform", "translateX(-75%)");

	var str = timedb().count() + " timers and"  +　tododb().count() + " todos";
	$(".me.wrapper .about-me .desc").html(str);
	
	if (!getCurrentUser()) { //未登陆，直接邀请注册
		$(".me.wrapper .about-me .title").html("Not Logon");
		$(".me.wrapper .about-me").attach(function() {
			pageRegister();
		});
		$(".me.wrapper li.backup .desc").html("No backups");
		$(".me.wrapper .logout").hide();
	} else {
		$(".me.wrapper .about-me").unbind();
		$(".me.wrapper .about-me .title").html(getCurrentUser());
		
		$(".me.wrapper li.backup .desc").html("Can not connect to server");
...
function pageDiscover() {
	...
	$(".discover .box").attach(function() {
		var ref = container.open('http://www.luckyna.com/dido/comment.html', '_blank', 'location=yes');
	});
	
	$(".discover .ing").attach(function() {
		var ref = container.open('http://www.luckyna.com/dido/ing.html?' + new Date().getTime(), '_blank', 'location=yes');
	});
	
	$(".discover .live").attach(function() {
		var ref = container.open('http://www.luckyna.com/dido/clock.html?' + new Date().getTime(), '_blank', 'location=yes');
	});
}
...
function pageCal() {
	...
	$(".content").css("-webkit-transform", "translateX(-25%)");
	$(".content").css("transform", "translateX(-25%)");
	timeCal(new Date());
